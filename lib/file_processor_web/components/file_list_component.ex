    ...
    <div :if={@processing_started or @all_done} class="bg-white rounded-2xl shadow-sm border overflow-hidden animate-in slide-in-from-bottom-4 duration-700">
      ...
      <div class="p-5 border-b bg-gray-50/50 flex flex-col gap-4">
        ...
        <div class="flex gap-2">
          <%= for {label, filter_val} <- [{"All", "all"}, {"Completed", "ok"}, {"Errors", "error"}] do %>
            <button
              type="button"
          ...
          <li id={"file-#{file.name}"} class="border-b border-gray-50 last:border-0 overflow-hidden transition-all duration-300">
            <div class="p-5 flex justify-between items-center hover:bg-gray-50/50">
              <div class="flex items-center gap-3">
                <div class={"w-2.5 h-2.5 rounded-full #{if file.status == :ok, do: "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]", else: "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]"}"}></div>
            ...
            <%!-- ERRORS INSPECTOR --%>
            <div :if={@expanded_file == file.name} class="bg-red-50/50 px-12 py-4 border-t border-red-100 animate-in slide-in-from-top-2 duration-300">
               <p class="text-[11px] text-red-600 font-mono">
                 <%= Map.get(file, :reason, "Unexpected system failure during parsing.") %>
               </p>
            </div>
          </li>
        <% end %>
